...
/* ─── SUPABASE SQL (ejecutar en Supabase SQL Editor) ─────────────────────────
 
-- 1. Catálogo de integraciones disponibles
CREATE TABLE integration_catalog (
  ...
  color TEXT,
  description TEXT,
  required_keys JSONB,  -- [{"key":"API Key","secret":true}, ...]
  docs_url TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 2. Credenciales cifradas del usuario
CREATE TABLE api_credentials (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  integration_id TEXT REFERENCES integration_catalog(id),
  key_name TEXT NOT NULL,
  encrypted_value TEXT NOT NULL,  -- pgcrypto: pgp_sym_encrypt(value, secret)
  is_active BOOLEAN DEFAULT TRUE,
  connected_at TIMESTAMPTZ DEFAULT NOW(),
  last_used_at TIMESTAMPTZ,
  last_used_by TEXT,  -- agent_id que la usó
  use_count INTEGER DEFAULT 0
);

-- 3. Log de uso de credenciales por agentes
CREATE TABLE credential_usage_log (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  integration_id TEXT,
  agent_id TEXT,
  agent_name TEXT,
  action TEXT,  -- 'check', 'used', 'failed'
  metadata JSONB,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 4. Función que Jarvis llama para verificar si tiene credenciales
CREATE OR REPLACE FUNCTION check_credential(p_integration_id TEXT)
RETURNS JSONB AS $$
DECLARE
  result JSONB;
  cred_count INTEGER;
BEGIN
  ...
  ELSE
    result := jsonb_build_object(
      'available', FALSE,
      'integration_id', p_integration_id,
      'message', 'No credentials found. User must connect this integration.',
      'action_required', 'OPEN_CONNECTIONS_PANEL',
      'panel_filter', p_integration_id
    );
  END IF;
  RETURN result;
END;
...
-- 5. Función para obtener valor descifrado (solo llamar desde servidor)
CREATE OR REPLACE FUNCTION get_credential(p_integration_id TEXT, p_key_name TEXT, p_secret TEXT)
RETURNS TEXT AS $$
BEGIN
  RETURN (
    SELECT pgp_sym_decrypt(encrypted_value::bytea, p_secret)
    FROM api_credentials
...
-- RLS: solo el owner puede ver sus credenciales
ALTER TABLE api_credentials ENABLE ROW LEVEL SECURITY;
CREATE POLICY "owner_only" ON api_credentials FOR ALL USING (auth.uid() IS NOT NULL);
...
const INTEGRATIONS = [
  // IA & Modelos
  { id:"openai",    name:"OpenAI",          cat:"ia",            icon:"◆",  color:"#10A37F", desc:"GPT-4o, DALL·E, Whisper, Embeddings",  keys:[{k:"API Key",s:true},{k:"Organization ID",s:false}],  status:"connected", popular:true },
  { id:"gemini",    name:"Google Gemini",   cat:"ia",            icon:"✦",  color:"#4285F4", desc:"Gemini Ultra, Pro, Flash",              keys:[{k:"API Key",s:true},{k:"Project ID",s:false}],        status:"connected", popular:true },
  ...
  // Infraestructura
  { id:"supabase",  name:"Supabase",        cat:"infra",         icon:"⬡",  color:"#3ECF8E", desc:"DB, Auth, Storage y Realtime",          keys:[{k:"Project URL",s:false},{k:"Anon Key",s:true},{k:"Service Role Key",s:true}], status:"connected", popular:true },
...
const JARVIS_MESSAGES = [
  ...
  { type:"success", text:"OpenAI y Gemini activos. Routing de modelos operacional." },
  { type:"info",    text:"Supabase conectado. Vault de credenciales cifrado y operativo." },
];
...
export default function JarvisConnections() {
  ...
  const handleConnect = () => {
    ...
    setTimeout(() => {
      ...
      setJarvisLog(p => [{
        type:"success",
        text:`${selected.name} conectado exitosamente. Jarvis ahora tiene acceso a esta integración.`
      }, ...p]);
  ...
  const isConn = (id) => statuses[id] === "connected";
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display:"flex", alignItems:"center", gap:20 }}>
          {/* Logo */}
          <div style={{ fontFamily:"'Syne', sans-serif", fontWeight:800, fontSize:15, letterSpacing:"3px", color:"#7B7BFF" }}>
        ...
        <div style={{
          ...
          <div style={{ flex:1, overflowY:"auto" }}>
            {CATS.map(c => {
              ...
              const connCount = c.id==="all" ? connected.length : connected.filter(i=>i.cat===c.id).length;
